...
export const updateEvent = mutation({
  ...
  handler: async (ctx, args) => {
    ...
    //  Get current event ot check tickets sold
    ...
    const soldTickets = await ctx.db
      ...
      .collect();
    // Ensure new total tickets is ot less than sold tickets
...
export const purchaseTicket = mutation({
  ...
  handler: async (ctx, { eventId, userId, waitingListId, paymentInfo }) => {
    // verify the waiting list entry exists and is valid
    ...
    try {
      ...
      // processing queue for next person
